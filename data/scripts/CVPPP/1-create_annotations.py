...
ANN_DIR = os.path.join(DATA_DIR, 'raw')
...
for image_path in image_paths:
    ...
    sem_annotation_path = os.path.join(ANN_DIR, image_name + '_fg.png')

    if not os.path.isfile(annotation_path):
        continue

    annotation = np.array(Image.open(annotation_path))
    sem_annotation = np.array(Image.open(sem_annotation_path))
