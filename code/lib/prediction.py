...
class Prediction(object):
    ...
    def __init__(self, resize_height, resize_width, mean,
        ...
        self.image_standarder = ImageUtilities.image_ex_standarder()
    ...
    def predict(self, image_path):
        ...
        if self.model.use_instance_segmentation:
            ...
            sem_seg_prediction = self.model.predict(image)
            sem_seg_prediction = sem_seg_prediction.squeeze(0)
            sem_seg_prediction = sem_seg_prediction[1]
            sem_seg_prediction = sem_seg_prediction.data.numpy()
            sem_seg_prediction = self.upsample_prediction(
                sem_seg_prediction, image_height, image_width)
            raw_image_pil = ImageUtilities.read_image(image_path)
            raw_image = np.array(raw_image_pil)
            return raw_image, sem_seg_prediction



    def predict_wae(self, image_path):
    ...
    def predict_attend(self, image_path):
